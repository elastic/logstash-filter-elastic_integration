...
if [[ "$INTEGRATION" != "true" ]]; then
  bundle exec rspec spec/filters --format=documentation --tag ~integration --tag ~secure_integration
else
  ...
  bundle exec rspec --format=documentation $extra_tag_args --tag update_tests:painless --tag es_version:$ES_VERSION spec/filters/integration
fi
