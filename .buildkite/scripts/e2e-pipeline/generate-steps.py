...
def generate_steps_for_target_branch(versions, target_release_version) -> list:
    steps: list = []
    
    releases = versions.get("releases", {})
    full_stack_version = releases.get(target_release_version)
    if full_stack_version is None:
        print(f"ERROR: No release version found for branch '{target_release_version}' in versions yaml", file=sys.stderr)
        sys.exit(1)
    steps.append(generate_test_step(full_stack_version, target_release_version, "false"))
    return steps
...
if __name__ == "__main__":
    ...
    target_release_version = os.getenv("TARGET_RELEASE_VERSION")

    if bk_source == "schedule" or bk_branch == "main":
        # a scheduler runs on all branches
        steps = generate_steps_for_scheduler(versions_yaml)
    elif bk_source == "ui" and bk_branch and bk_branch.startswith("pull/") and target_release_version:
        # a manual kick off on a PR branch
        steps = generate_steps_for_target_branch(versions_yaml, target_release_version)
    else:
