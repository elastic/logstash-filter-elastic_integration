...
NOTE: Plugin can not verify user privileges and runs with _unsafe_ mode when connecting to security disabled {es} cluster. This may lead unexpected pipeline runtime failure(s).
      Enabling security in {es} and using user authentication is recommended.

At the startup phase, plugin confirms that current user has sufficient privileges to _safely_ resolve ingest pipelines and able to execute their processors.
The required minimum privileges are:
...
When you install integrations like Nginx and Apache HTTP Server, they create `logs-nginx.access-` and `logs-apache.access-` similar ingest pipelines.
These ingest pipelines consist of one or multiple processors. When running the plugin for integration ingest pipelines, make sure that the plugin supports integration processors.
...
| `set_security_user` | Limited access to {es} internal authentication to retrieve user information
| `inference` | Limited access to {es} analytics or NLP models
| `enrich` | Limited ability to execute {es} enrich policies
...
When configured _without_ an explicit <<plugins-{type}s-{plugin}-pipeline_name>>, it also uses {es} to establish mappings of data stream names to their respective default pipeline names.
