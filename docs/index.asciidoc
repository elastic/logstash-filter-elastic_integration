...
Since this plugin embeds {es} ingest node components, there are situations where {es} introduces breaking changes that affect older plugin versions.
...
*Resolution:* When connecting to {es} 9.2 or newer, update this plugin to at least v9.2 to ensure compatibility with the updated ingest pipeline model.
