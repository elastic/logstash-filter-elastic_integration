...
* A JKS- or PKCS12-formatted Keystore (see <<plugins-{type}s-{plugin}-keystore>>)
...
* A JKS- or PKCS12-formatted Keystore(see <<plugins-{type}s-{plugin}-truststore>>)
...
Using this setting enables SSL if <<plugins-{type}s-{plugin}-ssl>> is not specified or this cannot be used with <<plugins-{type}s-{plugin}-ssl>> 'false'.
...
Host entry protocols decide enabling the SSL if <<plugins-{type}s-{plugin}-ssl>> is not specified. Set HTTPS to each entry when using SSL or HTTP when not using.
Not specifying entry protocols (example `"127.0.0.1"`) will cause normalizing the host protocols based on <<plugins-{type}s-{plugin}-ssl>> value, HTTP if `ssl => false`, HTTPS otherwise.
...
* it is explicitly disabled with `ssl => false`; OR
* it is implicitly disabled by providing `http`-protocol <<plugins-{type}s-{plugin}-hosts>>.

Specifying `ssl => true` can be a helpful redundant safeguard to ensure this plugin cannot be configured to use non-ssl communication.
...
If not specified and SSL is enabled, the verification mode becomes `full`. Refer to <<plugins-{type}s-{plugin}-ssl>> to get more information about when SSL can be enabled.
