...
* SSL is enabled when `cloud_id` is provided or all `hosts` entries are either HTTPS protocol or empty
* SSL is disabled when `cloud_id` isn't provided, all `hosts` entries are either HTTP protocol or empty
